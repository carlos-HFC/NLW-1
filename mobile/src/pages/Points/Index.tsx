...
const Points = () => {
   ...
   //BUSCAR ITENS DE COLETA
   useEffect(() => {
      api.get('/items')
         .then(({ data }) => setItems(data))
   }, [])

   //CARREGAR A POSIÇÃO ATUAL DO USUÁRIO
   useEffect(() => {
      async function loadPosition() {
         const { status } = await Location.requestPermissionsAsync()

         if (status !== 'granted') {
            Alert.alert('Oopss....', 'Precisamos de sua localização para prosseguir')
         }
            ...
            <View style={styles.mapContainer}>
               {
                  initialPosition[0] !== 0 && (
                     <MapView style={styles.map} initialRegion={{ latitude: initialPosition[0], longitude: initialPosition[1], latitudeDelta: 0.014, longitudeDelta: 0.014 }}>
                        {
                           points.map(point => (
                              <Marker key={String(point.id)} style={styles.mapMarker} onPress={() => navigation.navigate('Detail', { point_id: point.id })} coordinate={{ latitude: point.latitude, longitude: point.longitude }}>
